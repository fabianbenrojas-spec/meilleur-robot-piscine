...
for (const r of parseCsv(readFileSync(csvPath, "utf8"))) {
  ...
  const product = {
    ...
    type: r.type,
    specs: {
      surfaceMaxM2: S(num(r.surface_max_m2)),
      longueurMaxBassinM: S(num(r.longueur_max_bassin_m)),
      profondeurMaxM: S(num(r.profondeur_max_m)),
      zonesNettoyees: S(list(r.zones_nettoyees)),
      autonomieMin: S(num(r.autonomie_min)),
      tempsChargeMin: S(num(r.temps_charge_min)),
      cyclesMin: S(null),
      longueurCableM: S(num(r.longueur_cable_m)),
      cableAntitorsion: S(null),
      debitFiltrationLh: S(num(r.debit_filtration_lh)),
      finesseFiltrationMicrons: S(num(r.finesse_filtration_microns)),
      capaciteFiltreL: S(num(r.capacite_filtre_l)),
      typeFiltre: S(r.type_filtre || null),
      typeBrosse: S(r.type_brosse || null),
      poidsKg: S(num(r.poids_kg)),
      puissanceW: S(num(r.puissance_w)),
      pilotage: S(list(r.pilotage)),
      sortieDeLEauAutonome: S(null),
      chariotInclus: S(bool(r.chariot_inclus)),
      garantieMois: S(num(r.garantie_mois)),
    },
    compatibilite: {
      revetements: S(list(r.revetements)),
      formes: S(list(r.formes)),
      horsSol: S(bool(r.hors_sol)),
      enterree: S(bool(r.enterree)),
      fondPlat: S(null),
      penteComposee: S(bool(r.pente_composee)),
      escalier: S(bool(r.escalier)),
    },
